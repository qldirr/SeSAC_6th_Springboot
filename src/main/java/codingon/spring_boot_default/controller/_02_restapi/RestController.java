...
public class RestController {
    // ==================Template 렌더링===================
    // GET localhost:PORT/ 요청 시 _02_restapi/req.html이 브라우저에 렌더링
    ...
    public String getRes1(@RequestParam(value = "name") String name, @RequestParam(value = "age") int age, Model model) {
        ...
        // - query string 중에서 name key에 대한 value를 string name 에 매핑
        // - required=true 가 기본이므로 요청 URL에서 name key를 필수로 보내야함
        // Quiz) name input은 빈 input으로 요청을 보내도 에러가 발생하지않는 이유??
        // --> String은 빈문자열도 유효한 값으로 취급하기 때문
...
//        @PathVariable 어노테이션
        ...
        // - 기본적으로 path variable 은 값을 가져야함(즉 값이 없으면 404 Error)

        // 참구) URL 의 path variable과 해당 메서드의 매개변수명을 다르게 사용하고 싶다면??
...
//        path variable 중에서 name(필수), age(선택) 이라면??
        // - required = false 사용
        // - optional 한 변수가 맨뒤에 와야함

        // 참구) age 변수의 타입이 int 가 아닌 Integer 인 이유??
        // - age(숫자형) optional한 값이므로 null 이 가능
        // - primitive type(int) 은 null 값 불가
        // - 따라서 reference type 인 wrapper 객체 사용
    ...
    public String postRes1(@RequestParam String name, @RequestParam int age, Model model) {
        System.out.println("[GET] request request param (name) = " + name);
        System.out.println("[GET] request request param (age) = " + age);
    ...
    // 하지만 Spring boot를 API 서버로 활용하고자 데이터 자체를 응답하고 싶으면??
    // ==> @ResponseBody 사용
    // #7. Post Method/@RequestBody 어노테이션
...
// #16-1. VO with Get Method, Axios
    ...
    public String getRes5(@PathVariable String name, Model model) {

        model.addAttribute("name", name);

        return "_02_restapi/practice";
    ...
    @GetMapping("/post/practice")
    public String getPracitce(Model model) {
        return "_02_restapi/practice3";

    }

    @PostMapping("/post/practice")
    public String postPracitce(@ModelAttribute UserForm userForm, Model model) {
